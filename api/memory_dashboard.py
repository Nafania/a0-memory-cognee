...
from plugins._memory_cognee.helpers.memory import Memory, get_existing_memory_subdirs, get_context_memory_subdir, read_data_item_content
...
class MemoryDashboard(ApiHandler):
    ...
    async def process(self, input: dict, request: Request) -> dict | Response:
        try:
            from plugins._memory_cognee.helpers.memory import _get_cognee
            _get_cognee()
    ...
    async def _search_memories(self, input: dict) -> dict:
        try:
            ...
            try:
                import cognee
                from plugins._memory_cognee.helpers.memory import _extract_metadata_from_text
    ...
    async def _get_cognify_status(self) -> dict:
        try:
            from plugins._memory_cognee.helpers.cognee_background import CogneeBackgroundWorker
