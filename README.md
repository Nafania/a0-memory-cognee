...
- **Cognee 1.0.9** — uses Cognee V2 APIs where beneficial (`improve()`, `forget()`, `run_startup_migrations()`) and is exact-pinned because Cognee storage and dependency migrations can affect production data.
...
- `pip install cognee[fastembed]==1.0.9` (exact-pinned in `requirements.txt`)
